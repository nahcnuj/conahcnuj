...
| `.github/actions/owner-approved-auto-merge/action.yml` | owner 承認後に auto-merge を有効化する composite action | イベントは利用側ワークフローで `pull_request_review` を購読。書き込みには利用側の `GITHUB_TOKEN` を使用 |
